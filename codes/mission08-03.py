...
def displayImage():
    ...
    for i in range(dispH):
        ...
        for k in range(dispW):
            r = g = b = outImage[int(i/dispH*outH)][int(k/dispH*outH)]
...
def histoImageWithoutMatplot(): #TODO: 라벨과 축 붙이기
    global window, canvas, paper, filename, inImage, outImage, inH, inW, outH, outW
    # 히스토그램 형성
    inCountList = [0] * 256
    outCountList = [255] * 256
    ...
    # 히스토그램 출력
    FrameH = 2  # 그래프 바깥의 프레임 영역
    FrameW = 2  # 그래프 바깥의 프레임 영역

    histoColor = 0 # 0 => black
    histoH = 600
    histoW = 600
    outH = FrameH + histoH
    outW = FrameW + histoW

    barWidth = int(histoW / (len(inCountList)*2)) # 2를 곱한 이유는 공백을 그래프 폭과 같게 하기 위함임
    barMaxHeight = max(inCountList) - min(inCountList)


    ###### 메모리 할당 ################
    outImage = []
    outImage = malloc(outH, outW, initValue=255)
    ####### 진짜 컴퓨터 비전 알고리즘 #####
    for k in range(len(inCountList)):
        curBarH = int(inCountList[k] / barMaxHeight * histoH)
        for m in range(curBarH):
            for i in range(barWidth):
                outImage[FrameH+(histoH-m-1)][FrameW+(i+k*barWidth*2)] = histoColor
...
def uploadDetailRawImage():
    ...
    fsize = os.path.getsize(filename)
    height = width = int(math.sqrt(fsize))
    avgVal, maxVal, minVal = getImageDetails(filename)
    now = datetime.datetime.now()
    upDate = now.strftime('%Y-%m-%d')
    upUser = USER_NAME
...
if __name__ == "__main__":
    ...
    comVisionMenu2.add_command(label="히스토그램(matplotlib없이)", command=histoImageWithoutMatplot)
    ...
    mainMenu.add_cascade(label="데이터베이스", menu=comVisionMenu5)
    comVisionMenu5.add_command(label="RAW 이미지 일괄 저장(DB)", command=uploadRawImage)
    comVisionMenu5.add_command(label="RAW 이미지 일괄 저장(DB) + (평균, 최대값, 최소값)", command=uploadDetailRawImage)
    # comVisionMenu5.add_command(label="DB에서 가져오기", command=embossImage)
