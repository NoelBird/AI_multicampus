...
#                 C4 = inImage[iH + 1][iW]
#                 newValue = C1 * (1 - y) * (1 - x) + C2 * (
#                     1 - y) * x + C3 * y * x + C4 * y * (1 - x)
...
#             ys = k
#             xd = int(
#                 math.cos(radian) * (xs - cx) - math.sin(radian) *
#                 (ys - cy)) + cx
#             yd = int(
#                 math.sin(radian) * (xs - cx) + math.cos(radian) *
#                 (ys - cy)) + cy
...
#         normalCountList[i] = (outCountList[i] - minVal) * High / (
#             maxVal - minVal)
#     ## 서브 윈도창 생성 후 출력
...
#     subCanvas.create_image(
#         (256 // 2, 256 // 2), image=subPaper, state='normal')
#
...
#         for k in range(inW):
#             outImage[i][k] = int(
#                 ((inImage[i][k] - minVal) / (maxVal - minVal)) * 255)
#
#     displayImage()
#
#
...
#                     S += mask[m][n] * tmpInImage[i + m - MSIZE // 2][
#                         k + n - MSIZE // 2]
...
# def saveTempImage():
#     global window, canvas, paper, filename, inImage, outImage, inH, inW, outH, outW
#     saveFp = tempfile.gettempdir() + "/" + str(random.randint(10000,
#                                                               99999)) + ".raw"
#     if saveFp == '' or saveFp == None:
#         return
#     print(saveFp)
...
if __name__ == "__main__":
    ...
    status = tkinter.Label(window, text='이미지 정보:', bd=1, relief=tkinter.SUNKEN, anchor=tkinter.W)
