...
def waveHorOpenCV():
    ...
    for i in range(inH):
        for k in range(inW):
            oy = int(15.0* math.sin(2*3.14*k / 100)) # 15는 적당히 준거임. 인터넷에서 검색해보면 됨
            ox = 0
            if i+oy < inH:
...
def waveVirOpenCV():
    ...
    for i in range(inH):
        for k in range(inW):
            ox = int(15.0 * math.sin(2 * 3.14 * i / 100))  # 15는 적당히 준거임. 인터넷에서 검색해보면 됨
            oy = 0
            if k + ox < inW:
...
def cartoonOpenCV():
    ...
    photo2 = cv2.cvtColor(mask, cv2.COLOR_GRAY2RGB)
    toColorOutArray(photo2)
